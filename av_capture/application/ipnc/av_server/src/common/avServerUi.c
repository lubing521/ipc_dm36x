...
void UI_setConfig(AVSERVER_Config *config)
{
    ...
    {
        config->captureRawInMode	= gAVSERVER_UI_config.saldreEnable ? 
                                      AVSERVER_CAPTURE_RAW_IN_MODE_DDR_IN : 
                                      AVSERVER_CAPTURE_RAW_IN_MODE_ISIF_IN;
    }
    ...
    if (SensorIsYuvMode() > 0)
    {
        ...
        config->aewb_config.aewbEnable = FALSE;
    }
    else
    {
        config->aewb_config.aewbVendor = AEWB_TI;
        config->aewb_config.aewbEnable = TRUE;
    }
	...
	case AVSERVER_UI_CAPTURE_MODE_720P_D1:
	{
        ...
        case AR0130:
        case MT9M034:
            config->sensorMode = DRV_IMGS_SENSOR_MODE_1280x840;
            break;
        case IMX122:
