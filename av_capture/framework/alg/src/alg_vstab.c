...
IALG_Handle VS_ALG_create(IALG_Fxns *fxns,IALG_Handle p,IALG_Params *params)
{
    ...
    IALG_Handle alg;
    IALG_Fxns *fxnsPtr;
    int n = fxns->algAlloc(params, &fxnsPtr, memTab);
    if (n <= 0)
    {
        return(NULL);
    }
...
void *ALG_vstabCreate(ALG_VstabCreate *create)
{
    ...
    ALG_VsObj *pObj = OSA_memAlloc(sizeof(ALG_VsObj));
    if (pObj == NULL)
    {
        return NULL;
    }
...
int ALG_vstabRun(void *hndl, ALG_VstabRunPrm *prm, ALG_VstabStatus *status)
{
    ...
    IVS1_Fxns *ivsFxns = (IVS1_Fxns *)(pObj->vsHandle)->fxns;
