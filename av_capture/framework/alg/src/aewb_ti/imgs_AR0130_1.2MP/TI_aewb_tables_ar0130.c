                             ...
                             unsigned char *win_coeffs)
{
    ...
    for (i = 0; i < height1; i ++)
    {
        for (j = 0; j < width1; j ++)
        {
            if (i >= v_start2 && i < (v_start2 + height2) && j >= h_start2 && j < (h_start2 + width2))
            {
...
short TI_2A_SetEEValues_ar130(int shift_val) //shift=3 for 1080P/720P, shift=4 for D1
{
...
void TI_2A_init_tables_ar130(int width, int height)
{
#ifdef AEWB_PRINTS
