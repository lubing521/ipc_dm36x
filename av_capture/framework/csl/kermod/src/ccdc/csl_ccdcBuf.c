...
{
    CSL_Status status = CSL_bufInit(&hndl->dataOutBuf, bufInit);
...
{
    CSL_Status status = CSL_bufGetFull(&hndl->dataOutBuf, buf, minBuf, timeout);
    return status;
}

CSL_Status CSL_ccdcBufPutEmpty(CSL_CcdcHandle hndl, CSL_BufInfo * buf)
{
    CSL_Status status = CSL_bufPutEmpty(&hndl->dataOutBuf, buf);
...
{
    CSL_Status status = CSL_bufInit(&hndl->lscBuf, bufInit);
...
{
    CSL_Status status = CSL_bufGetFull(&hndl->lscBuf, buf, minBuf, timeout);
...
{
    CSL_Status status = CSL_bufPutEmpty(&hndl->lscBuf, buf);
