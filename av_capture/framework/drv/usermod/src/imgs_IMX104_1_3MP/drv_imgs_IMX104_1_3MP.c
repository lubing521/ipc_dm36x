...
#define IMX104_SPI_DRIVER_MODE 1
...
int DRV_imgsSetRegs(Uint16 width, Uint16 height, int fps)
{
    ...
    //      {
    status =
        DRV_i2cRead8(&gDRV_imgsObj.i2cHndl, lvdsRegAddr, lvdsRegValue, 5);
    //Iss_deviceRead8(gImx104Lvds_I2c_ctrl.i2cInstId,0x2d,lvdsRegAddr,lvdsRegValue, 5);
    OSA_printf("lvds324 chip id 1st:0x%x\n", lvdsRegValue[0]);
    OSA_printf("defaut width:0x%x,height:0x%x",
               (lvdsRegValue[1] << 8) | (lvdsRegValue[2]),
               (lvdsRegValue[3] << 8) | (lvdsRegValue[4]));
    //      }while(1);
    ...
    lvdsRegAddr[1] = 0x0A;
    lvdsRegValue[1] = 0x62;     /*0x62--->SCLK:58MHz~75MHz;OUTCLK:116~150MHz */
    /*0x61--->SCLK:31MHz~60MHz;OUTCLK:62~120MHz */
    status =
        DRV_i2cWrite8(&gDRV_imgsObj.i2cHndl, lvdsRegAddr, lvdsRegValue, 2);
#endif
    /*for sensor register */
    InitRegArrary();
