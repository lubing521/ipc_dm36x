...
void imx122_setgio(unsigned short addr, unsigned short reg)
{
    int i;
    unsigned char imx122_value;
    unsigned char spi_chipid;
    unsigned char page_addr;
    unsigned char regset[3];
    spi_chipid = (unsigned char)(addr >> 8);
    page_addr = (unsigned char)(addr&0xFF);
    imx122_value =( unsigned char) reg;

    regset[0] = spi_chipid;
    regset[1] = page_addr;
    regset[2] = imx122_value;
...
#ifdef  IMX22_SPI_TEST
    do{
        imx122_setgio(IMX122_SETTING[0],0x25);      
    }while(1);	 
#endif

    //OSA_waitMsecs(200);
    //imx122_setgio(0x0200,0x01); //standy  mode
    //OSA_waitMsecs(200);

    for (i = 0; i < sizeof(IMX122_SETTING)/sizeof(unsigned short); i = i+2)
    {
        imx122_setgio(IMX122_SETTING[i],IMX122_SETTING[i+1]);      
    }

    OSA_waitMsecs(100);
    imx122_setgio(0x0200,0x30); //cancel standy mode
...
int DRV_imgsOpen(DRV_ImgsConfig *config)
{
    ...
    OSA_printf("sensor mode:%d;width:%d;height:%d;%s\n",config->sensorMode,width,height,__FUNCTION__);
...
int DRV_imgsSetEshutter(Uint32 eshutterInUsec, int setRegDirect)
{
    ...
    imx122_setgio(0x0209,Send_H); 
    imx122_setgio(0x0208,Send_L); 
    ...
    imx122_setgio(0x0206,Send_H); 
    imx122_setgio(0x0205,Send_L);
