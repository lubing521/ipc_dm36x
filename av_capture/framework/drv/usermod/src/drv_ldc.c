...
{
    int status;
    DRV_ImgsLdcConfig *pImgsLdcConfig = NULL;
    DRV_LdcObj *pObj = NULL;
...
int DRV_ldcImageTuneCmdExecuteLdc(IMAGE_TUNE_CmdInfo *cmdInfo, IMAGE_TUNE_CmdInfo *statusInfo)
{
    ...
    DRV_LdcObj *pObj = &gDRV_ldcObj[DRV_LDC_TUNING_TOOL_CTRL_STREAM_ID];

    int retVal = IMAGE_TUNE_CmdExecuteLdcPrm(cmdInfo, statusInfo);
    if (retVal == OSA_SOK)
    {
