...
int DRV_dmaTestDemux(Uint8 *srcVirtAddr, Uint8 *dstVirtAddr, int size, int width, int height)
{
    ...
    demuxPrm.srcPhysAddrList = (long unsigned int*)OSA_memAlloc( sizeof(unsigned long)*demuxPrm.numLines );
    demuxPrm.dstPhysAddrList = (long unsigned int*)OSA_memAlloc( sizeof(unsigned long)*demuxPrm.numLines );
