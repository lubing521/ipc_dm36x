...
int DRV_imgsCalcFrameRate_MT9P031(int fps)
{
    ...
    gDRV_imgsObj.curFrameTime.VB      = gDRV_imgsObj.curFrameTime.t_frame / gDRV_imgsObj.curFrameTime.t_row - gDRV_imgsObj.curFrameTime.H - 2;
...
int DRV_imgsCalcFrameTime_MT9P031(Uint32 fps, Uint32 dataW, Uint32 dataH, Bool binEnable)
{
    ...
    pFrame->f_pclk = 64 * IMGS_MHZ;//default value

    if (dataW*dataH >= 1280*720)
    {
    ...
    pFrame->pll_N   = (2 - 1);
    pFrame->pll_M   = (16);
    pFrame->pll_DIV = (IMGS_PLL_REF_CLK * (unsigned long)pFrame->pll_M) / ((pFrame->pll_N + 1) * pFrame->f_pclk) - 1;
    pFrame->t_pclk  = 1000.0*IMGS_MHZ/pFrame->f_pclk;
    pFrame->t_row   = 2 * pFrame->t_pclk * OSA_max(pFrame->W/2 + OSA_max(pFrame->HB, pFrame->HBmin), 41+346*(pFrame->row_bin+1)+99);
    ...
    DRV_imgsCalcSW_MT9P031(pFrame->t_frame / 1000);
...
int DRV_imgsCalcSW_MT9P031(int exposureTimeInUsecs)
{
    ...
    if (pFrame->SW > 10 * 1024)
    {
        pFrame->SW = 10 * 1024 - 1; 
    }
    
#if 0
    OSA_printf("\n ----exposureTimeInUsecs=%d,shutterOverhead=%f,t_row=%f,SW=%d", exposureTimeInUsecs,pFrame->shutterOverhead, pFrame->t_row, pFrame->SW);
#endif
...
int DRV_imgsInvertSW_MT9P031(int regVal)
{
    DRV_ImgsFrameTime *pFrame = &gDRV_imgsObj.curFrameTime;
    int exposureTimeInUsecs = (regVal*pFrame->t_row - pFrame->shutterOverhead) / 1000;
