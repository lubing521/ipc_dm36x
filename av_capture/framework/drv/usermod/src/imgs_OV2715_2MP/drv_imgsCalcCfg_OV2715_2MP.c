...
int DRV_imgsCalcAgain(int aGain)
{
    ...
    if(aGain > 8000*31/16) aGain = 8000*31/16;
    ...
    else if (aGain <= 2000*31/16)
    {
        gainRegVal = 0x10;
        aGain /= 2;
    }
    else if (aGain <= 4000*31/16)
    {
        gainRegVal |= 0x30;
        aGain /= 4;
    }
    ...
    gainRegVal |= (((aGain-1000)*16 + 500)/1000 & 0x0f);
    //OSA_printf(" aGain = %d gainRegVal = 0x%X\n", aGain, gainRegVal);

    return gainRegVal;
}

#if 1 //Gang: calc real exposure
