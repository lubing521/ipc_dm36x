...
int DRV_reszInit()
{
    ...
    {
        CSL_RszOutConfig *pOutConfig = &gDRV_reszObj.outConfig[i];
...
int DRV_reszCheckMagicNum(DRV_ReszRunPrm *pPrm, int bytesPerPixel, int chroma)
{
    ...
    for (i = 0; i < CSL_RSZ_CH_MAX; i++)
    {
        if (pPrm->pOut[i] != NULL)
        {
            ...
            {
                pAddr = (Uint32 *)((Uint32)pPrm->pOut[i]->outVirtAddr + pPrm->pOut[i]->outOffsetH * (pPrm->pOut[i]->outHeight - 2) * bytesPerPixel);
...
int DRV_reszRun(DRV_ReszRunPrm *pPrm)
{
    ...
    while (numIterations--)
    {
        ...
        else if (inType == DRV_DATA_FORMAT_YUV422)
        {
            bytesPerPixel[0] = 2;
            bytesPerPixel[1] = 1;
        }
        else if (inType == DRV_DATA_FORMAT_YUV420)
        {
            bytesPerPixel[0] = 1;
            bytesPerPixel[1] = 1;
        }
