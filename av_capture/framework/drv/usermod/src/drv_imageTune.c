...
int DRV_imageTuneLoadParams(int mode, int paramSetId)
{
    ...
    awb_calc_data_t         *pAwbCfg;
    ALG_vnfParams           *pVnfCfg;
    int retVal = IMAGE_TUNE_LoadParams(paramSetId);
    if (retVal == OSA_SOK)
    {
...
int DRV_imageTuneSaveParams(int mode, int paramSetId, Bool saveDefaultParams)
{
    ...
    DRV_ImgsIsifConfig  *pIsifCfg  = drvImgsFunc->imgsGetIsifConfig(mode);
    DRV_ImgsIpipeConfig *pIpipeCfg = drvImgsFunc->imgsGetIpipeConfig(mode, 0);
    DRV_ImgsLdcConfig   *pLdcCfg   = drvImgsFunc->imgsGetLdcConfig(mode, 0, 0);
    awb_calc_data_t     *pAwbCfg   = ImageTune_imgsGetAwbConfig(mode);
    ALG_vnfParams       *pVnfCfg   = ImageTune_imgsGetVnfConfig(mode);
...
int DRV_imageTuneLoadParamSet(int mode, int paramSetId){
    ...
    int retVal = DRV_imageTuneLoadParams( mode, paramSetId);
