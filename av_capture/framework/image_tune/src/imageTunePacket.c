...
{
    int i = 0;
    Uint32 *tmpPtr  = &packet[IMAGE_TUNE_COMMAND_ID_OFFSET];
    Uint32 size     = packet[IMAGE_TUNE_PRM_SIZE_OFFSET] + IMAGE_TUNE_PACKET_HEADER_SIZE - 2*sizeof(Uint32);
    Uint32 checkSum = 0;
    for (i = 0; i < size / 4 ; i++)
    {
        checkSum += *tmpPtr++;
...
{
    int retVal = OSA_SOK;
    IMAGE_TUNE_PacketHeader *pRecvPacketHeader = (IMAGE_TUNE_PacketHeader *)packet;
    Uint32 *tmpPtr;
    Uint32 checkSum;
