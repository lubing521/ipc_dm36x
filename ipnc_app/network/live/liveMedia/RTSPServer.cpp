...
void RTSPServer::addServerMediaSession(ServerMediaSession* serverMediaSession) {
	...
	ServerMediaSession* existingSession =
			(ServerMediaSession*) (fServerMediaSessions->Add(sessionName,
...
void RTSPServer::removeServerMediaSession(
...
char* RTSPServer::rtspURLPrefix(int clientSocket) const {
	...
	if (clientSocket < 0) {
		...
		ourAddress.sin_addr.s_addr
				= ReceivingInterfaceAddr != 0 ? ReceivingInterfaceAddr
	...
	if (portNumHostOrder == 554 /* the default port number */) {
		sprintf(urlBuffer, "rtsp://%s/PSIA/Streaming/channels/",
				our_inet_ntoa(ourAddress.sin_addr)); //PSIA/Streaming/channels/
	}
...
RTSPServer::RTSPClientSession::RTSPClientSession(RTSPServer& ourServer,
	...
	fOurServer(ourServer), fOurSessionId(sessionId),
			fOurServerMediaSession(NULL), fClientSocket(clientSocket),
			fClientAddr(clientAddr), fLivenessCheckTask(NULL),
			fIsMulticast(False), fSessionIsActive(True),
			fStreamAfterSETUP(False), fTCPStreamIdCount(0),
...
void RTSPServer::RTSPClientSession::incomingRequestHandler1() {
	...
	int bytesRead = readSocket(envir(), fClientSocket,
			&fRequestBuffer[fRequestBytesAlreadySeen], fRequestBufferBytesLeft,
			dummy);
	handleRequestBytes(bytesRead);
}

void RTSPServer::RTSPClientSession::handleAlternativeRequestByte(
...
void RTSPServer::RTSPClientSession::handleAlternativeRequestByte1(
	...
	if (fRequestBufferBytesLeft == 0 || fRequestBytesAlreadySeen
			>= RTSP_BUFFER_SIZE)
		return;
...
void RTSPServer::RTSPClientSession::handleRequestBytes(int newBytesRead) {
	...
	// Look for the end of the message: <CR><LF><CR><LF>	//RTSP请求结束标记,类似HTTP
	unsigned char *tmpPtr = ptr;
	if (fRequestBytesAlreadySeen > 0)
		--tmpPtr;
	...
	if (!endOfMsg)
		return; // subsequent reads will be needed to complete the request
	...
	if (!parseRTSPRequestString((char*) fRequestBuffer,
			fRequestBytesAlreadySeen, cmdName, sizeof cmdName, urlPreSuffix,
			sizeof urlPreSuffix, urlSuffix, sizeof urlSuffix, cseq, sizeof cseq)) {
#ifdef DEBUG
		...
		if (strcmp(cmdName, "OPTIONS") == 0) {
			...
			/*************************work around for milestone start*************************/
			if (urlSuffix[0] == '2')//channel 2 is for H264 in IPNC
			{
				sprintf(urlSuffix, "2?videoCodecType=H.264");
			}
			/************************* work around for milestone end**************************/

			handleCmd_DESCRIBE(cseq, urlSuffix, (char const*) fRequestBuffer);
		} else if (strcmp(cmdName, "SETUP") == 0) {
			handleCmd_SETUP(cseq, urlPreSuffix, urlSuffix,
					(char const*) fRequestBuffer);
		} else if (strcmp(cmdName, "TEARDOWN") == 0 || strcmp(cmdName, "PLAY")
				== 0 || strcmp(cmdName, "PAUSE") == 0 || strcmp(cmdName,
				"GET_PARAMETER") == 0 || strcmp(cmdName, "SET_PARAMETER") == 0) {
...
#endif
	send(fClientSocket, (char const*) fResponseBuffer,
...
static void parseTransportHeader(char const* buf, StreamingMode& streamingMode,
	...
	while (1) {
		if (*buf == '\0')
			return; // not found
		if (_strncasecmp(buf, "Transport: ", 11) == 0)
			break;
		++buf;
	}
...
void RTSPServer::RTSPClientSession::handleCmd_SETUP(char const* cseq,
	...
	if (fOurServerMediaSession != NULL && strcmp(streamName,
			fOurServerMediaSession->streamName()) != 0) {
	...
	if (fOurServerMediaSession == NULL) {
		...
		if (streamName[0] != '\0' || fOurServer.lookupServerMediaSession("")
				!= NULL) { // normal case
	...
	if (trackId != NULL && trackId[0] != '\0') { // normal case
		for (streamNum = 0; streamNum < fNumStreamStates; ++streamNum) {
			...
			if (subsession != NULL && strcmp(trackId, subsession->trackId())
					== 0)
				break;
		}
	...
	parseTransportHeader(fullRequestStr, streamingMode, streamingModeString,
			...
			clientRTPPortNum, clientRTCPPortNum, rtpChannelId, rtcpChannelId);
	...
	subsession->getStreamParameters(fOurSessionId, fClientAddr.sin_addr.s_addr,
			clientRTPPort, clientRTCPPort, tcpSocketNum, rtpChannelId,
			rtcpChannelId, destinationAddress, destinationTTL, fIsMulticast,
			serverRTPPort, serverRTCPPort, fStreamStates[streamNum].streamToken);
	...
	if (fIsMulticast) {
		...
		case RAW_UDP:
			snprintf((char*) fResponseBuffer, sizeof fResponseBuffer,
					...
					ntohs(serverRTPPort.num()), destinationTTL, fOurSessionId);
			break;
		}
	} else {
		switch (streamingMode) {
...
void RTSPServer::RTSPClientSession::handleCmd_withinSession(
	...
	ServerMediaSubsession* subsession;
	if (urlSuffix[0] != '\0' && strcmp(fOurServerMediaSession->streamName(),
			urlPreSuffix) == 0) {
...
void RTSPServer::RTSPClientSession::handleCmd_TEARDOWN(
		ServerMediaSubsession* /*subsession*/, char const* cseq) {
	snprintf((char*) fResponseBuffer, sizeof fResponseBuffer,
			"RTSP/1.0 200 OK\r\nCSeq: %s\r\n%s\r\n", cseq, dateHeader());
	fSessionIsActive = False; // triggers deletion of ourself after responding
}
...
Boolean RTSPServer::RTSPClientSession::authenticationOK(char const* cmdName,
		char const* cseq, char const* urlSuffix, char const* fullRequestStr) {

	if (!fOurServer.specialClientAccessCheck(fClientSocket, fClientAddr,
			urlSuffix)) {
		snprintf((char*) fResponseBuffer, sizeof fResponseBuffer,
	...
	do {
		...
		if (!parseAuthorizationHeader(fullRequestStr, username, realm, nonce,
				uri, response) || username == NULL || realm == NULL || strcmp(
				realm, fCurrentAuthenticator.realm()) != 0 || nonce == NULL
				|| strcmp(nonce, fCurrentAuthenticator.nonce()) != 0 || uri
				== NULL || response == NULL) {
			break;
		}
	...
	snprintf((char*) fResponseBuffer, sizeof fResponseBuffer,
			...
			cseq, dateHeader(), fCurrentAuthenticator.realm(),
...
RTSPServer::createNewClientSession(unsigned sessionId, int clientSocket,
		struct sockaddr_in clientAddr) {
...
RTSPServer::ServerMediaSessionIterator::ServerMediaSessionIterator(
		RTSPServer& server) :
			fOurIterator(
					(server.fServerMediaSessions == NULL) ? NULL
