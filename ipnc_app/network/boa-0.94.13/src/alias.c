...
#ifdef DAVINCI_IPCAM
    if (strncasecmp(req_urip, JPEG_FRAME_URI, sizeof(JPEG_FRAME_URI)-1) == 0) {
        extern HTTP_URI *HttpUri;

        req->http_uri = HttpUri;
        return 1;
    }
    req->http_uri = http_uri_search(req_urip);
    if (req->http_uri && (req->http_uri->uri_flag & URI_FLAG_VIRTUAL_PAGE)) {
        return 1;
    }
#endif
...
#ifdef PSIA_MEDIA
     if (strncasecmp(req_urip, "/PSIA", 5) == 0) 
         req->psia_uri = psia_uri_search(req_urip); 
   
	//fprintf(stderr,"\r\n psia_uri : %s\n",req->psia_uri);
    if (req->psia_uri && (req->psia_uri->psia_uri_flag & PSIA_URI_FLAG_VIRTUAL_PAGE)) {
        return 1;
    }
#endif

    uri_len = strlen(req_urip);
